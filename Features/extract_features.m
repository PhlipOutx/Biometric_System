function features = extract_features(id, variables, input, output, imagelist, datadir, savefeatures, outputdir)
	...
	if strcmp(variables.feature.name,'LBP') || strcmp(variables.feature.name,'HOG') || strcmp(variables.feature.name,'LPQ') || strcmp(variables.feature.name,'WLD')  || strcmp(variables.feature.name,'LCH')
		features = extract_local_features(id, variables, input, output, imagelist, datadir, savefeatures, outputdir);
	elseif strcmp(variables.feature.name,'PCA') || strcmp(variables.feature.name,'LDA') || strcmp(variables.feature.name,'KFA') || strcmp(variables.feature.name,'KPCA') || strcmp(variables.feature.name,'ICA')
		features = extract_subspace_features(id, variables, input, output, imagelist, datadir, savefeatures, outputdir);
	elseif strcmp(variables.feature.name,'SIFT') || strcmp(variables.feature.name,'SURF')
		features = extract_keypoint_features(id, variables, input, output, imagelist, datadir, savefeatures, outputdir);
	elseif strcmp(variables.feature.name,'GABOR')
		features = extract_gabor_features(id, variables, input, output, imagelist, datadir, savefeatures, outputdir);
	end
	
end


function features = extract_subspace_features(id, variables, input, output, imagelist, datadir, savefeatures, outputdir)
...
function features = extract_local_features(id, variables, input, output, imagelist, datadir, savefeatures, outputdir)
    ...
    if ~isempty(savefeatures) && exist([outputdir id savefeatures],'file')
		...
		elseif strcmp(variables.feature.name,'HOG')
			for i = 1:inum
				...
				for j = 1:patches.num
					H1 = hog(I(patches.y(j):patches.dy(j),patches.x(j):patches.dx(j)), variables.feature.bins);
                    H1 = (H1/sum(H1));
					% NaN fix for GOH
					H1(isnan(H1)) = 0;
...
function features = extract_keypoint_features(id, variables, input, output, imagelist, datadir, savefeatures, outputdir)
...
function features = extract_gabor_features(id, variables, input, output, imagelist, datadir, savefeatures, outputdir)
