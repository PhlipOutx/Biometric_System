...
model.dim = NumOfSampl;
model.varcap = 0;

%[mixedsig, model.P] = remmean(mixedsig);
model.P = mean(mixedsig,1)';
mixedsig = mixedsig-repmat(model.P',Dim,1);
...
if only > 2
  ...
  if ~isempty(W)
    ...
    %model.W = W * mixedsig + (W * model.P) * ones(1, NumOfSampl);
	model.W = (W * mixedsig + W * repmat(model.P,1,size(W,2))')';
    %icasig = W * mixedsig;
    %if b_verbose & (max(abs(W * mixedmean)) > 1e-9) & (strcmp(displayMode,'signals') | strcmp(displayMode,'on'))
