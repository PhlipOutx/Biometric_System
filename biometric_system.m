...
% results:          See Section 3.0 for list of outputs.
    ...
    if strcmp(experiment.variables.distance,'libsvm')
        ...
        experiment.variables.pids = textscan(fid, '%d');
        fclose(fid);
        experiment.variables.pids = double(experiment.variables.pids{1});
        
        fid = fopen(experiment.input.gallery_id);
        experiment.variables.gids = textscan(fid, '%d');
        fclose(fid);
        experiment.variables.gids = double(experiment.variables.gids{1});
    end
    t1 = tic;
