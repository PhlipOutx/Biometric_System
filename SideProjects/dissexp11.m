function results = dissexp11(input)
...
% results:			See Section 3.0 for list of outputs.
	...
	% results = load([experiment.output.resultsdir w1id 'distance.mat'], 'distances'); distances = results.distances; clearvars results;
	% results = load([experiment.output.resultsdir w3id 'distance.mat'], 'distances'); distances = distances + results.distances; clearvars results;
	% results = load([experiment.output.resultsdir w5id 'distance.mat'], 'distances'); distances = distances + results.distances; clearvars results;
	% results = load([experiment.output.resultsdir w7id 'distance.mat'], 'distances'); distances = distances + results.distances; clearvars results;
	% results = load([experiment.output.resultsdir w9id 'distance.mat'], 'distances'); distances = distances + results.distances; clearvars results;
	...
	for k = 1:2
		for j=1:5
			...
			for i=0:20
				...
				EERs(i+1) = ROC_rates_and_threshs.EER_er*100;
				fprintf('[%3.2f w][%3.2f%%]',(i*.25),ROC_rates_and_threshs.EER_er*100);
			end
			fprintf('\n');
			[C,I] = min(EERs);
			weights(j) = (I-1)*0.25;
			time = toc(t2);
			fprintf('[%6.2f sec][%3.2f%% score][k=%d][j=%d] %3.2f %3.2f %3.2f %3.2f %3.2f\n\n',time,C,k,j,weights)
		end
	end
