function results = dissexp8(input)
...
%%%%%%%%%%%%%%%%%%%%%
%%%%	unweighted score based fusion selection - Miller dissertation patches - Miller disseration feature extraction method for trained classifier - on periocular using FRGC data
%%%%%%%%%%%%%%%%%%%%%
	...
	fprintf('##Computing best feature\n');
			...
			for j=1:4
				...
				if j ~= output(i)
					if j == 1
						...
						dj = compute_distances(gallery_features, probe_features, experiment.id, experiment.variables, experiment.input, experiment.output);
						dj = (dj-min(min(dj)))/(max(max(dj))-min(min(dj)));
					elseif j == 2
					...
					if i==1,distances=dj+d2+d3+d4+d5+d6+d7;end
					if i==2,distances=d1+dj+d3+d4+d5+d6+d7;end
					if i==3,distances=d1+d2+dj+d4+d5+d6+d7;end
					if i==4,distances=d1+d2+d3+dj+d5+d6+d7;end
					if i==5,distances=d1+d2+d3+d4+dj+d6+d7;end
					if i==6,distances=d1+d2+d3+d4+d5+dj+d7;end
					if i==7,distances=d1+d2+d3+d4+d5+d6+dj;end
					...
					if ROC_rates_and_threshs.VER_01FAR_ver*100 > tops
						tops = ROC_rates_and_threshs.VER_01FAR_ver*100;
						if i==1,d1=dj;end
						if i==2,d2=dj;end
						if i==3,d3=dj;end
						if i==4,d4=dj;end
				...
				time = toc(t2);
				fprintf('[%6.2f s][k=%d][i=%d][j=%d] %3.2f \n',time,k,i,j,ROC_rates_and_threshs.VER_01FAR_ver*100);
