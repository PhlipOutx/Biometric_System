function results = dissexp9(input)
...
%%%%%%%%%%%%%%%%%%%%%
%%%%	weighted structural locations - Miller dissertation patches - Miller disseration feature extraction method for untrained classifier - on periocular using FRGC data
%%%%%%%%%%%%%%%%%%%%%
	...
	gallery_features =  gf4(((loc(i,1)-1)*flength(j))+1:loc(i,2)*flength(j),:);
	probe_features =  pf4(((loc(i,1)-1)*flength(j))+1:loc(i,2)*flength(j),:);
	d2 = compute_distances(gallery_features, probe_features, experiment.id, experiment.variables, experiment.input, experiment.output);
	...
	gallery_features =  gf4(((loc(i,1)-1)*flength(j))+1:loc(i,2)*flength(j),:);
	probe_features =  pf4(((loc(i,1)-1)*flength(j))+1:loc(i,2)*flength(j),:);
	d3 = compute_distances(gallery_features, probe_features, experiment.id, experiment.variables, experiment.input, experiment.output);
	...
	gallery_features =  gf4(((loc(i,1)-1)*flength(j))+1:loc(i,2)*flength(j),:);
	probe_features =  pf4(((loc(i,1)-1)*flength(j))+1:loc(i,2)*flength(j),:);
	d4 = compute_distances(gallery_features, probe_features, experiment.id, experiment.variables, experiment.input, experiment.output);
	...
	gallery_features =  gf4(((loc(i,1)-1)*flength(j))+1:loc(i,2)*flength(j),:);
	probe_features =  pf4(((loc(i,1)-1)*flength(j))+1:loc(i,2)*flength(j),:);
	d5 = compute_distances(gallery_features, probe_features, experiment.id, experiment.variables, experiment.input, experiment.output);
	...
	gallery_features =  gf4(((loc(i,1)-1)*flength(j))+1:loc(i,2)*flength(j),:);
	probe_features =  pf4(((loc(i,1)-1)*flength(j))+1:loc(i,2)*flength(j),:);
	d6 = compute_distances(gallery_features, probe_features, experiment.id, experiment.variables, experiment.input, experiment.output);
	...
	gallery_features =  gf4(((loc(i,1)-1)*flength(j))+1:loc(i,2)*flength(j),:);
	probe_features =  pf4(((loc(i,1)-1)*flength(j))+1:loc(i,2)*flength(j),:);
	d7 = compute_distances(gallery_features, probe_features, experiment.id, experiment.variables, experiment.input, experiment.output);
	...
	for k = 1:2
		for j=1:7
			...
			for i=0:20
				if j==1
					distances = ((i*.1)*d1)+(weights(2)*d2)+(weights(3)*d3)+(weights(4)*d4)+(weights(5)*d5)+(weights(6)*d6)+(weights(7)*d7);
				elseif j==2
					distances = (weights(1)*d1)+((i*.1)*d2)+(weights(3)*d3)+(weights(4)*d4)+(weights(5)*d5)+(weights(6)*d6)+(weights(7)*d7);
				elseif j==3
					distances = (weights(1)*d1)+(weights(2)*d2)+((i*.1)*d3)+(weights(4)*d4)+(weights(5)*d5)+(weights(6)*d6)+(weights(7)*d7);
				elseif j==4
					distances = (weights(1)*d1)+(weights(2)*d2)+(weights(3)*d3)+((i*.1)*d4)+(weights(5)*d5)+(weights(6)*d6)+(weights(7)*d7);
				elseif j==5
					distances = (weights(1)*d1)+(weights(2)*d2)+(weights(3)*d3)+(weights(4)*d4)+((i*.1)*d5)+(weights(6)*d6)+(weights(7)*d7);
				elseif j==6
					distances = (weights(1)*d1)+(weights(2)*d2)+(weights(3)*d3)+(weights(4)*d4)+(weights(5)*d5)+((i*.1)*d6)+(weights(7)*d7);
				elseif j==7
					distances = (weights(1)*d1)+(weights(2)*d2)+(weights(3)*d3)+(weights(4)*d4)+(weights(5)*d5)+(weights(6)*d6)+((i*.1)*d7);
				%elseif j==8
				%	distances = (weights(1)*d1)+(weights(2)*d2)+(weights(3)*d3)+(weights(4)*d4)+(weights(5)*d5)+(weights(6)*d6)+(weights(7)*d7)+((i*.1)*d8);
				end
				
				CMC_rec_rates = compute_cmc(experiment.id, distances, experiment.variables, experiment.input, experiment.output);
				Ranks(i+1) = CMC_rec_rates(1)*100;
				fprintf('[%3.2f w][%3.2f%%]',(i*.1),CMC_rec_rates(1)*100);
				...
				%Ranks(i+1) = ROC_rates_and_threshs.VER_01FAR_ver*100;
				%fprintf('[%3.2f w][%3.2f%%]',(i*.1),ROC_rates_and_threshs.VER_01FAR_ver*100);
			end
			fprintf('\n');
			[C,I] = max(Ranks);
			weights(j) = (I-1)*0.1;
			...
			fprintf('[%6.2f sec][%3.2f%% score][k=%d][j=%d] %3.2f %3.2f %3.2f %3.2f %3.2f %3.2f %3.2f \n',time,C,k,j,weights)
		end
	end
	...
	for j=1:7
		...
		for i=1:5
			if j==1
				distances = ((weights(1)+((i-3)*0.05))*d1)+(weights(2)*d2)+(weights(3)*d3)+(weights(4)*d4)+(weights(5)*d5)+(weights(6)*d6)+(weights(7)*d7);
			elseif j==2
				distances = (weights(1)*d1)+((weights(2)+((i-3)*0.05))*d2)+(weights(3)*d3)+(weights(4)*d4)+(weights(5)*d5)+(weights(6)*d6)+(weights(7)*d7);
			elseif j==3
				distances = (weights(1)*d1)+(weights(2)*d2)+((weights(3)+((i-3)*0.05))*d3)+(weights(4)*d4)+(weights(5)*d5)+(weights(6)*d6)+(weights(7)*d7);
			elseif j==4
				distances = (weights(1)*d1)+(weights(2)*d2)+(weights(3)*d3)+((weights(4)+((i-3)*0.05))*d4)+(weights(5)*d5)+(weights(6)*d6)+(weights(7)*d7);
			elseif j==5
				distances = (weights(1)*d1)+(weights(2)*d2)+(weights(3)*d3)+(weights(4)*d4)+((weights(5)+((i-3)*0.05))*d5)+(weights(6)*d6)+(weights(7)*d7);
			elseif j==6
				distances = (weights(1)*d1)+(weights(2)*d2)+(weights(3)*d3)+(weights(4)*d4)+(weights(5)*d5)+((weights(6)+((i-3)*0.05))*d6)+(weights(7)*d7);
			elseif j==7
				distances = (weights(1)*d1)+(weights(2)*d2)+(weights(3)*d3)+(weights(4)*d4)+(weights(5)*d5)+(weights(6)*d6)+((weights(7)+((i-3)*0.05))*d7);
			%elseif j==8
			%	distances = (weights(1)*d1)+(weights(2)*d2)+(weights(3)*d3)+(weights(4)*d4)+(weights(5)*d5)+(weights(6)*d6)+(weights(7)*d7)+((i*.1)*d8);
			end
			...
			CMC_rec_rates = compute_cmc(experiment.id, distances, experiment.variables, experiment.input, experiment.output);
			Ranks(i+1) = CMC_rec_rates(1)*100;
			fprintf('[%3.2f w][%3.2f%%]',(weights(j)+((i-3)*0.05)),CMC_rec_rates(1)*100);
			
			%[true_scores false_scores] = split_true_false_scores(experiment.id, distances, experiment.variables, experiment.input, experiment.output);
			%[~, ~, ROC_rates_and_threshs] = compute_roc(true_scores, false_scores, experiment.output.roc_resolution);
			%Ranks(i+1) = ROC_rates_and_threshs.VER_01FAR_ver*100;
			%fprintf('[%3.2f w][%3.2f%%]',(weights(j)+((i-3)*0.05)),ROC_rates_and_threshs.VER_01FAR_ver*100);
		end
		fprintf('\n');
		[C,I] = max(Ranks);
		weights(j) = (weights(j)+((I-4)*0.05));
		time = toc(t2);
		
		fprintf('[%6.2f sec][%3.2f%% score][j=%d] %3.2f %3.2f %3.2f %3.2f %3.2f %3.2f %3.2f \n',time,C,j,weights)
	end
	...
	distances = (weights(1)*d1)+(weights(2)*d2)+(weights(3)*d3)+(weights(4)*d4)+(weights(5)*d5)+(weights(6)*d6)+(weights(7)*d7);
	s=whos('distances');
