function results = demographic_classifier_vote(input)
...
% results:			See Section 3.0 for list of outputs.
	...
	results = load([experiment.output.resultsdir experiment.LBP256id experiment.output.results_mat], 'results'); LBP256results = results.results; clearvars results;
	results = load([experiment.output.resultsdir experiment.HOG12id experiment.output.results_mat], 'results'); HOG12results = results.results; clearvars results;
	results = load([experiment.output.resultsdir experiment.LPQ256id experiment.output.results_mat], 'results'); LPQ256results = results.results; clearvars results;
	results = load([experiment.output.resultsdir experiment.LCH16id experiment.output.results_mat], 'results'); LCH16results = results.results; clearvars results;
	...
	for i=1:5
        LBP59pred =  LBP59results.prediction(:,i);
		LBP256pred = LBP256results.prediction(:,i);
		HOG12pred =  HOG12results.prediction(:,i);
		LPQ256pred = LPQ256results.prediction(:,i);
		LCH16pred =  LCH16results.prediction(:,i);
		test_ids =  LBP59results.truth(:,i);
		
		pred = mode([LBP59pred, LBP256pred, HOG12pred, LPQ256pred, LCH16pred]')';
