function results = GAonDissV3(input)
...
%
%
% This is the experiment code for producing the best features as selected by the GA that are used in chapter 3 of version 3 of my dissertation. 
% If you are changing between Rank1 and EER remember to switch the > and <


    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % parse experiment xml file
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    start_t = tic;
    if isstruct(input)
        experiment = input;
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % print out some of the xml file
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    fprintf('###Beginning GAonDissV3 [%s]\n', experiment.id);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % non-GA experiment features
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    loc = [1,7;8,12;13,16;17,20;21,28;29,34;35,74];
    subregion = 7; % variable that decides which subregion this is working with
    experiment.variables.loc = loc;
    experiment.variables.subregion = subregion;
    experiment.variables.patchlength = loc(subregion,2) - loc(subregion,1) + 1;
    flength = 0;
    ...
    gallery_features =  gallery_features(((loc(subregion,1)-1)*flength)+1:loc(subregion,2)*flength,:);
    probe_features =  probe_features(((loc(subregion,1)-1)*flength)+1:loc(subregion,2)*flength,:);
    
    fprintf('##Performing experiment on subregion: %d\n', subregion);
    fprintf('##Using patches %d - %d\n', loc(subregion,1), loc(subregion,2));
    fprintf('##Total of %d patches\n', experiment.variables.patchlength);
    fprintf('##Total feature vector length of %d\n', experiment.variables.patchlength*flength);
    fprintf('##Trimmed probe feature matrix [%d features x %d images] [%6.2f MB]\n', size(probe_features,1), size(probe_features,2), s.bytes/1000000);
    fprintf('##Trimmed gallery feature matrix [%d features x %d images] [%6.2f MB]\n', size(gallery_features,1), size(gallery_features,2), s.bytes/1000000);
    ...
    if ~isempty(experiment.output.population_mat) && exist([experiment.output.resultsdir experiment.id experiment.output.population_mat],'file')
        ...
        for i=1:numchrom
            ...
            if i < 10
                fprintf('Chromosome  %d - %7.4f%%:  ', i, scores(i));
            else
                fprintf('Chromosome %d - %7.4f%%:  ', i, scores(i));
            end
            for z = 1:length(chromosome)
                fprintf('%d', chromosome(z));
            end
            fprintf('\n');
    ...
    for actiter = actiter:numiter
        ...
        if same == 0
            ...
            printing(experiment.variables.printing, '[the child was already in the population]', actiter, numiter);
        end
    ...
    for z = 1:length(best_chromosome)
        fprintf('%d', best_chromosome(z));
    end
    fprintf('\n');
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    repchrom = repmat(best_chromosome,1,experiment.variables.patchlength);
    chrom = find(repchrom==1);
...
function score = getScore(chromosome, probe_features, gallery_features, id, variables, input, output)
    repchrom = repmat(chromosome,1,variables.patchlength);
    chrom = find(repchrom==1);
    
    probe_features=probe_features(chrom,:);
    gallery_features=gallery_features(chrom,:);
    distances = slmetric_pw(gallery_features, probe_features, variables.distance);
    CMC_rec_rates = compute_cmc(id, distances, variables, input, output);
    score = CMC_rec_rates(1)*100;
    clearvars distances CMC_rec_rates;
end
